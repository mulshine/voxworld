...
#define TRIG_INIT(m,k,c,s)                                       \
     {                                                           \
      int i;                                                     \
      for (i=2 ; i<=k ; i++)                                     \
          {m->coswrk[i]=m->costab[i];m->sinwrk[i]=m->sintab[i];} \
      t_lam = 0;                                              \
      c = 1;                                                     \
      s = 0;                                                     \
     }
#define TRIG_NEXT(m,k,c,s)                                         \
     {                                                           \
         int i,j;                                                \
         (t_lam)++;                                              \
         for (i=0 ; !((1<<i)&t_lam) ; i++);                 \
         ...
         if (i>1)                                                            \
            {                                                                \
             ...
             j         = k - j;                                              \
             m->sinwrk[i] = m->halsec[i] * (m->sinwrk[i-1] + m->sinwrk[j]);  \
             m->coswrk[i] = m->halsec[i] * (m->coswrk[i-1] + m->coswrk[j]);  \
            }                                                                \
     }
...
{
    _tMempool* mempool = *mp;
    
    _tMayer* m = *mayer = (_tMayer*) mpool_calloc(sizeof(_tMayer), mempool);
    
    m->mempool = mempool;
    
    m->halsec = (float*)mpool_alloc(sizeof(float)*20,mempool);
    m->costab = (float*)mpool_alloc(sizeof(float)*20,mempool);
    m->sintab = (float*)mpool_alloc(sizeof(float)*20,mempool);
    m->coswrk = (float*)mpool_alloc(sizeof(float)*20,mempool);
    m->sinwrk = (float*)mpool_alloc(sizeof(float)*20,mempool);
    
    for (int i = 0; i < 20; i++)
    {
        m->halsec[i] = ghalsec[i];
        m->costab[i] = gcostab[i];
        m->sintab[i] = gsintab[i];
        m->coswrk[i] = gcoswrk[i];
        m->sinwrk[i] = gsinwrk[i];
    }
}
...
void mayer_free(tMayer* const mayer)
{
    ...
    mpool_free((char*)m->halsec, m->mempool);
    mpool_free((char*)m->costab, m->mempool);
    mpool_free((char*)m->sintab, m->mempool);
    mpool_free((char*)m->coswrk, m->mempool);
    mpool_free((char*)m->sinwrk, m->mempool);
}

void mayer_fht(tMayer* const mayer, REAL *fz, int n)
{
    _tMayer* m = *mayer;
...
void mayer_fft(tMayer* const mayer, int n, REAL *real, REAL *imag)
{
     REAL a,b,c,d;
     REAL q,r,s,t;
     int i,j,k;
     ...
     mayer_fht(mayer,real,n);
     mayer_fht(mayer,imag,n);
}

void mayer_ifft(tMayer* const mayer, int n, REAL *real, REAL *imag)
{
     REAL a,b,c,d;
     REAL q,r,s,t;
     int i,j,k;
...
void mayer_realfft(tMayer* const mayer, int n, REAL *real)
{
    REAL a,b;
    int i,j,k;
    mayer_fht(mayer,real,n);
...
void mayer_realifft(tMayer* const mayer, int n, REAL *real)
{
    REAL a,b;
    int i,j,k;
    for (i=1,j=n-1,k=n/2;i<k;i++,j--) {
